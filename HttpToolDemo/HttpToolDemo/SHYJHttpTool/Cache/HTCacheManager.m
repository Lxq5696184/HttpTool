...
+ (HTCacheManager *)shareManager {
    static HTCacheManager * manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (manager == nil) {
            manager = [[HTCacheManager alloc] init];
        }
    });
    return manager;
}
...
- (void)cacheResponseObject:(id)responseObject
    ...
    if (error == nil) {
        //缓存到内存中
        [HTMemoryCache writeData:responseObject forKey:hash];
        ...
        if (!directoryPath) {
            directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"HttpTool"] stringByAppendingPathComponent:@"newworkCache"];
            [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:cacheDirKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        [HTDiskCache writeData:data toDir:directoryPath filename:hash];
        [[HTLRUManager shareManager] addFileNode:hash];
    }
}
- (id)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
    ...
    //先从内存中查找
    cacheData = [HTMemoryCache readDataWithKey:hash];
    if (!cacheData) {
        NSString * directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath) {
            cacheData = [HTDiskCache readDataFormDir:directoryPath filename:hash];
            if (cacheData) {
                [[HTLRUManager shareManager] refreshIndexOfFileNode:hash];
            }
        }
    }
    return  cacheData;
...
- (void)storeDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"HttpTool"] stringByAppendingPathComponent:@"download"];
        [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:downloadDirKey];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
    }
    [HTDiskCache writeData:data toDir:directoryPath filename:fileName];
}
...
- (NSURL *)getDownloadDataFromCacheWithRequestUrl:(NSString *)requestUrl {
    ...
    if (directoryPath) {
        data = [HTDiskCache readDataFormDir:directoryPath filename:fileName];
    }
    if (data) {
...
- (NSUInteger)totalCacheSize {
    ...
    return [HTDiskCache dataSizeInDir:diretoryPath];
}
...
- (void)clearDownloadData {
    NSString * directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:downloadDirKey];
    [HTDiskCache clearDataDir:directoryPath];
}

- (NSString *)getDownloadDiretoryPath {
    NSString * directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:downloadDirKey];
...
- (void)clearTotalCache {
    NSString*directoeyPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
    [HTDiskCache clearDataDir:directoeyPath];
}

- (void)clearLRUCache {
    if ([self totalCacheSize] > diskCapacity) {
        NSArray * deleteFiles = [[HTLRUManager shareManager] removeLRUFileNodeWithCacheTime:cacheTime];
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath && deleteFiles.count > 0) {
            [deleteFiles enumerateObjectsUsingBlock:^(NSString * obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [HTDiskCache deleteCache:filePath];
            }];
        }
    }
}
